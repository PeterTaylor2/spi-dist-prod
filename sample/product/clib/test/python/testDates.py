...
raw_input("Importing clib - enter to continue:")
import clib

class MyClass(clib.Object):
    ...
    @staticmethod
    def from_dict(m):
        return MyClass(m["name"], m["age"])

clib.Object.register_class(MyClass, "MyClass")
...
class TestDates(unittest.TestCase):
    ...
    def testMyClass(self):
        o = MyClass("Peter", 42)
        encoding = o.to_string()
        o2 = clib.Object.from_string(encoding)
        assert(o != o2)
        assert(o2.to_string() == encoding)
    def testDayOfWeek(self):
        """Tests clib.DayOfWeek function"""
        date = datetime.date(2014,10,4) # which is Saturday
        print "DayOfWeek(%s) = %s" % (date, clib.DayOfWeek(date))
        for dow in ["Sunday", "Monday", "Tuesday", "Wednesday",
